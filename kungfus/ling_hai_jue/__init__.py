from kungfus.ling_hai_jue.attribute import Attribute
from kungfus.ling_hai_jue.buffs import BUFFS
from kungfus.ling_hai_jue.dots import DOTS
from kungfus.ling_hai_jue.gains import GAINS
from kungfus.ling_hai_jue.recipes import RECIPE_CHOICES, RECIPES
from kungfus.ling_hai_jue.skills import SKILLS
from kungfus.ling_hai_jue.talents import TALENT_CHOICES, TALENTS


def prepare(self, player_id):
    pass
